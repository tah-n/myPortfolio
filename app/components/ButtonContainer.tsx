...
const ButtonContainer = () => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
     if(moveArrow) {
      if(e.clientX/window.innerWidth < 0.99 && e.clientX/window.innerWidth > 0.6) {
        if (e.clientY/window.innerHeight > 0.3 && e.clientY/window.innerHeight < 0.4) {
        ...
        else if (e.clientY/window.innerHeight > 0.4 && e.clientY/window.innerHeight < 0.5) {
          gsap.to(arrowRef.current, {
            y: 88,
            duration: 0.5,
            ease: 'power2.out' 
        ...
        else if (e.clientY/window.innerHeight > 0.55 && e.clientY/window.innerHeight < 0.61) {
          gsap.to(arrowRef.current, {
            y: 170,
            duration: 0.5,
            ease: 'power2.out' 
  ...
  return (
    <div className='absolute right-3 top-32' onMouseEnter={() => setMoveArrow(true)} onMouseLeave={() => setMoveArrow(false)}>
